...
model = torch.hub.load('lpdr/detection/yolov5', 'custom', path='lpdr/detection/weigths/best_yolo_cbam_v2.pt', source='local')

def get_plates_image(image):

    df_results = model(image).pandas().xyxy[0]
    df_detections = df_results[df_results["confidence"] > 0.1]
